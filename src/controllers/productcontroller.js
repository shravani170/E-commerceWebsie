...
const createProduct = async function (req, res) {
    try {
        ...
        const isTitleAlreadyUsed = await productModel.findOne({ title });//checks uniqueness of title
    ...
    try {
        ...
        if (validate.isValidRequestBody(queryParams)) {
            ...
            if (validate.isValid(name)) {
                ...
                filterQuery['title']['$options'] = '$i'//will fetch small as wll as capital i/p
            }
...
const updateProduct=async function (req, res) 
{
    ...
    let {title, description, price,isFreeShipping,style,availableSizes} = updateBody
    ...
    // }
    if (Object.prototype.hasOwnProperty.call(updateBody, 'availableSizes')) {
        var Check = availableSizes.split(",")
  ...
  //  checkProductId.save();
    let UpdateData = {title,description,price,isFreeShipping,productImage:uploadedFileURL,style,availableSizes}
    let updatedProduct = await productModel.findOneAndUpdate({ _id:reqProductId }, UpdateData, { new: true })
